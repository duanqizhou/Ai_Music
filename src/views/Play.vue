<template>
  <div>
    ...
    <div class="wrap">
      <h4>{{ song.name || "由Ai音乐提供" }}</h4>
      <h5>{{ song.artist || "---" }}</h5>

      <van-row class="mt20" justify="space-between">
        <van-col class="fz-16" span="6">标准</van-col>
        <van-col class="fz-16" span="6">音效</van-col>
        <van-col class="fz-16" span="6">封面</van-col>
      </van-row>
      <div class="flex" @click="changePicOrLyricHandle">
        <transition v-show="changePicOrLyric" name="fade">
          <van-image
            v-if="showPic"
            radius="10px"
            width="7rem"
        ...
        <transition v-show="!changePicOrLyric" name="fade">
          ...
          >
            <ul>
              <li v-for="(item,index) in lyricS" :key="item" :ref="lyricLine" :class="{active:lyricLinesIndex === index}">
                {{ item.txt }}
              </li>
              <li v-for="heel in 3" :key="heel">&nbsp;</li>
            </ul>
          </div>
        </transition>
      </div>

      <div class="opt-wrap mt30 mb20">
        <van-image
          v-for="(item, index) in opt"
          :key="index"
          width="0.53333rem"
          fit="contain"
          :src="require('@/assets/' + item.com_img)"
          @click="imageOptClick(index, $e)"
        />
      </div>

      <div class="voice mt30 flex-bet ">
        <span>
          {{
            audioGenerateTime._currentTime == "NaN:NaN"
              ? "00:00"
              : audioGenerateTime._currentTime
          }}
        </span>
        <van-slider
          v-model="value"
          @change="onChange"
          active-color="#FFB622FF"
        />
        <span>{{
          audioGenerateTime._duration == "NaN:NaN"
            ? "00:00"
            : audioGenerateTime._duration
        }}</span>
      </div>

      <div class="fullw80 mt30 flex-bet">
        <svg class="icon prev" @click="prevSongH">
          <use xlink:href="#icon-prev"></use>
        </svg>
        <transition name="fade">
          <svg class="icon play" @click="changePlayH">
            <use xlink:href="#icon-pause" v-if="pause"></use>
            <use xlink:href="#icon-play" v-else></use>
          </svg>
        </transition>

        <svg class="icon next" @click="nextSongH">
    ...
    >
      <van-empty
        v-if="commentList && commentList.length < 1"
        class="custom-image"
        image="https://img.yzcdn.cn/vant/custom-empty-image.png"
        description="暂无评论，快去发布第一条评论吧~"
      />
      <div id="comment">
        <div class="flex item" v-for="(item,index) in commentList" :key="index">
          <van-image
          round
          width="50"
          height="50"
          ...
          <div class="flex-ali-c flex1">
            <div class="flex1">
              ...
              <div class="mt10 fz-12 mr5">
                {{item.content}}
              </div>
            </div>
            <div>
              <van-icon name="like" v-if="item.liked" @click="likeContent(song.id,item,0)"/>
              <van-icon name="like-o" v-else  @click="likeContent(song.id,item,1)"/>
            </div>
          </div>
        </div>
      </div>
...
import { _songCommentLike } from '../server/apis'
export default {
  components: {},
  setup () {
    const _ = inject('_')
    ...
    const changePicOrLyric = ref(false)
    ...
    const lyricLinesIndex = ref(0)
    const commentList = ref([])
    const betterScrollInstance = ref(null)
    const LyricInstance = ref(null)
    const currentLyric = ref('')
    const store = useStore()
    const setSong = (song) => {
      if (song && song.value.source) {
        audio.src = song.value.source
      }
    }
    const musics = store.state.music
    const Ml = musics.length
    if (Ml < 1) return
    let musicIndex = 0
    const song = ref(musics[musicIndex])
    const audio = new Audio()
    audio.loop = true
    setSong(song)
    const shapeFlag = ref(false)
    ...
    const onPercentBarChange = (percent) => {
      ...
      const currentTime = audio.duration * (percent / 100)
    ...
    const generateTime = () => {
      ...
      value.value = (audio.currentTime / audio.duration) * 100
      if (value.value >= 100) {
        audio.currentTime = 0
      }
    }
    ...
    const prevSongH = () => {
      ...
      musicIndex = index
      song.value = musics[index]
    }
    const nextSongH = () => {
      showPic.value = false
      musicIndex += 1
      const index = musicIndex % Ml
      musicIndex = index
      song.value = musics[index]
    }

    // 歌曲操作条
    const imageOptClick = (index, e) => {
      switch (index) {
        case 0:
          break
        case 3:
          ...
          if (commentList?.value?.length <= 1 && song.value?.commentList) {
            commentList.value = song.value.commentList
          }
          break
    ...
    const scrollToLyric = (indexOrNum) => {
      if (!betterScrollInstance.value) {
        Toast.fail('betterScroll Init Fail')
        return
      }
      switch (typeof indexOrNum) {
        case 'string':
        case 'number':
          lyricLinesIndex.value = indexOrNum
          betterScrollInstance.value.scrollTo(
            0,
            -Math.abs((indexOrNum - 3) * lyricHeight),
            500
          )
          break

          /* ????? */
          // betterScrollInstance.value.scrollTo(0, -Math.abs(indexOrNum), 500)
          // break
        default:
          Toast('传入的定位歌词参数失败')
          break
    ...
    const lyricHandle = () => {
      let lastHeight = null
      return ({ lineNum, txt }) => {
        if (!lastHeight) lastHeight = -3 * lyricHeight
        if (Number(lineNum) > 4) {
          // let lineEl = lyricLines[lineNum - 3];
          if (lineNum <= lyricLines.value.length) {
            nextTick(() => {
              scrollToLyric(lineNum)
              lastHeight = -Math.abs(Number(lineNum) * lyricHeight)
            })
          }
        } else {
          // if (this.$refs.lyricList) {
          //   this.$nextTick(() => {
          //     betterScrollInstance.value.scrollTo(0, 0, 1000);
          //   });
          // }
        }
    ...
    const changePicOrLyricHandle = () => {
      changePicOrLyric.value = !changePicOrLyric.value
    }
    const likeContent = async (id, cid, type) => {
      if (id && cid) {
        const _cid = cid.commentId
        const hasSuc = await _songCommentLike(id, _cid, type)
        if (hasSuc.code !== 200) {
          Toast('点赞暂时失败稍后再试~')
        }
      } else {
    ...
    const audioGenerateTime = computed(() => {
      let durmin = Math.floor(duration.value / 60)
      let dursec = Math.floor(duration.value - durmin * 60)
      let curmin = Math.floor(currentTime.value / 60)
      let cursec = Math.floor(currentTime.value - curmin * 60)
      if (durmin < 10) {
        durmin = '0' + durmin
      }
      if (dursec < 10) {
      ...
      return {
        _duration: durmin + ':' + dursec,
        _currentTime: curmin + ':' + cursec
      }
    })
    // 进入缓存页面触发
    const lyricS = ref([])
    onActivated(() => {
      const musics = store.state.music
      const firstSong = ref(musics[0])
      if (firstSong.value.sign !== song.value.sign) {
        song.value = firstSong.value
        setSong(firstSong)
      }
      if (song.value?.lyricT) {
        LyricInstance.value = new Lyric(song.value.lyricT, lyricHandle())
      }
      if (LyricInstance.value?.lines?.length > 0) {
        lyricS.value = LyricInstance.value.lines
      }
    })
    ...
    nextTick(() => {
      betterScroll.value &&
        (betterScrollInstance.value = new BScroll(betterScroll.value, {
          scrollY: true
        }))
    })

    watch(song, (newValue, oldValue) => {
      if (oldValue.source === newValue.source) return
      Toast({
        duration: 0,
        forbidClick: true,
        message: '加载中...'
      })
      audio.src = newValue.source
      audio.currentTime = 0
      value.value = 0
      setTimeout(() => {
        try {
...
.wrap {
  ...
  .lyric {
    ...
    width: 7rem;
    height: 7rem;
    background: linear-gradient(#30261cff, #614a39ff);
  }
  #betterScroll {
    overflow: hidden;
    pointer-events: none;
